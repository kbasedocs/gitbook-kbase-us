...
2. **File 1** - This column should be the name of the file including the extension and path to the file, if it is different from the CSV. 
3. **File 2** \( as needed\) - If a second file is needed, for instance when importing a .gff or non-interleaved reads. Use the same format of file name, path, and extension. 
